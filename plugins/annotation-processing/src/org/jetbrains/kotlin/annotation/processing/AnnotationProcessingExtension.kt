...
private fun AnalysisContext.analyzeDeclaration(declaration: PsiElement) {
    ...
    if (declaration is PsiClass) {
        declaration.allMethods.forEach { analyzeDeclaration(it) }
        declaration.allFields.forEach { analyzeDeclaration(it) }
        declaration.allInnerClasses.forEach { analyzeDeclaration(it) }
    }
